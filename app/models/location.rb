class Location < ApplicationRecord
  ...
  scope :finding_fiftykm_nearest_all_cabs, lambda { |latitude, longitude, clat,
    clong, cab_type|
    joins("JOIN cabs ON cabs.id = locations.cab_id")
      .where("latitude <= ?
             ...
             AND cabs.brand = ?",
             Arel.sql((latitude + 0.5).to_s),
             Arel.sql((latitude - 0.5).to_s),
             Arel.sql((longitude + 0.5).to_s),
  ...
  scope :finding_fiftykm_nearest_cab, lambda { |latitude, longitude, clat,
    clong, cab_type|
    joins("JOIN cabs ON cabs.id = locations.cab_id")
      .where("latitude <= ?
             AND latitude >= ?
             ...
             AND cabs.availability = true
             AND cabs.brand = ?",
             Arel.sql((latitude + 0.5).to_s), Arel.sql((latitude - 0.5).to_s),
             Arel.sql((longitude + 0.5).to_s),
             Arel.sql((longitude - 0.5).to_s), cab_type)
      .order(Arel.sql("((#{clat} - #{latitude}) * (#{clat} - #{latitude}))
      +((#{clong} - #{longitude}) * (#{clong} - #{longitude})) ASC"))
